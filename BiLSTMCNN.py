# -*- coding: utf-8 -*-
"""
BiLSTM + 1D CNN Regression (5-fold CV)
- BiLSTM 分支提取时序依赖
- 1D CNN 分支提取局部特征
- 拼接后回归预测
"""
...
# ===================== BiLSTM + CNN Model =====================
class BiLSTMCNNRegressor(nn.Module):
    def __init__(self, input_dim):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Conv1d(cnn_out_channels, cnn_out_channels, kernel_size=cnn_kernel_size, padding=1),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1)  # 输出 (B, cnn_out_channels, 1)
        )
        
        # Fully connected head
        self.fc = nn.Sequential(
            nn.Linear(2*lstm_hidden + cnn_out_channels, 128),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(128,1)
        )
    ...
    def forward(self, x):
        # BiLSTM branch
        lstm_out, _ = self.bilstm(x)  # (B, seq_len, 2*lstm_hidden)
        lstm_feat = lstm_out.mean(dim=1)
        
        # CNN branch
        cnn_in = x.permute(0, 2, 1)  # (B, F, seq_len)
        cnn_feat = self.cnn(cnn_in).squeeze(-1)  # (B, cnn_out_channels)
...
for fold, (tr, va) in enumerate(skf.split(X_scaled, y_strat),1):
    ...
    model = BiLSTMCNNRegressor(feat_dim).to(device)
...
df.to_csv("BiLSTM_CNN_cv_metrics.csv", index=False)
print("\n===== Final Results =====")
print(df)
